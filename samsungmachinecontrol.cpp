...
QString SamsungMachineControl::getWMIHWInfo(int type)
{
    /*
...
QString SamsungMachineControl::getMac()
{
    QString macAddress;
...
QString SamsungMachineControl::getKey(QString machineinfo, QString ddMMyyyy, int months, bool bindWithCP)
{
    ...
    if ( machineinfo.isEmpty() ) {
        ...
        if ( bindWithCP ) {
            originalStr120 = machineinfo + "QY_SHINE" + QString::number(2009);
        } else {
...
bool SamsungMachineControl::activeKey(QString key, bool bindWithPC)
{
    ...
    for ( int d = 0; d != daydelay; ++d )
    {
        for ( int i = 0; i <= kValidity.size(); ++i )
        {
            ...
            if ( bindWithPC ) {
                k0 = getKey(getInfo(), "QY_SHINE", 2009);
            } else {
                k0 = getKey(getInfo(), dtct.date().toString("ddMMyyyy"), validity);
            }
...
void SamsungMachineControl::copyToFile(QSettings* reg, bool magic)
{
...
bool SamsungMachineControl::initializeReg()
{
    QSettings *reg = new QSettings(kReg, QSettings::NativeFormat);
...
bool SamsungMachineControl::judgeFile()
{
    QSettings *reg = new QSettings(kReg, QSettings::NativeFormat);
...
bool SamsungMachineControl::judgeDate()
{
    QSettings *reg = new QSettings(kReg, QSettings::NativeFormat);
...
int SamsungMachineControl::judgeKey()
{
    QSettings *reg = new QSettings(kReg, QSettings::NativeFormat);
...
void SamsungMachineControl::refreshDT1()
{
    QSettings *reg = new QSettings(kReg, QSettings::NativeFormat);
